...
for command_name in awk cut find getent git python3 runuser sacctmgr sbatch \
...
if ! MODULE_PROBE=$(runuser -u "$BENCH_USER" -- env \
    ...
    bash --noprofile --norc -c '
        ...
        command -v apptainer
        set +u
        source activate flux-0.88.0
        set -u
        command -v flux
        printf "__WMSbench_FLUX_PREFIX__=%s\n" "${CONDA_PREFIX:?}"
    '); then
    echo "software probe failed for $BENCH_USER; verify the configured modules and flux-0.88.0 environment" >&2
    exit 1
fi
FLUX_ENV_PREFIX=$(awk -F= '$1=="__WMSbench_FLUX_PREFIX__" {print $2; exit}' \
    <<<"$MODULE_PROBE")
[[ -d $FLUX_ENV_PREFIX ]] || {
    echo "could not resolve the activated Mamba environment named flux-0.88.0" >&2
    exit 1
}
...
if ! runuser -u "$BENCH_USER" -- env MODULE_INIT="$MODULE_INIT" \
    MAMBA_MODULE=mamba bash --noprofile --norc -c '
        set -eo pipefail
        set +u
        source "$MODULE_INIT"
        set -u
    ...
    echo "could not capture the flux-0.88.0 environment manifest for $BENCH_USER" >&2
    exit 1
fi
