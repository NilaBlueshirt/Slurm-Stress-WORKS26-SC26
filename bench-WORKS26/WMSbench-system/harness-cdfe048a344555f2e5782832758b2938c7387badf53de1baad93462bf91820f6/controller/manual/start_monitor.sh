...
# margin fits before this venue's measured sdiag generation roll.  A manual run
# is not exempt: a counter delta that straddles the roll is unusable no matter
# who pressed the keys.
...
# and records its own PID.  Its duration is generous because a manual run ends
# when the operator marks the endpoint, not on a controller timer: a series
# that stops early would leave the secondary global-context audit short.
SAMPLER_DURATION=$((WMSbench_CAP_SECONDS + POST_BASELINE + CANCEL_GRACE + 4 * INTERVAL))
...
setsid bash -c '
    ...
    shift
    exec "$@"
' works26-sampler "$SAMPLER_PID_FILE" \
...
manual_state_put sampler_duration_s "$SAMPLER_DURATION"
...
monitoring is open for $VENUE rep$REP $BACKEND (collection position $ORDER of 5)
  ...
  sdiag sampler PID : $SAMPLER_PID (every ${INTERVAL}s, up to ${SAMPLER_DURATION}s)
...
step 3, watch for the LASTZ endpoint (run this as root, never as
