...
def _flux_node_transition_fields(root: Path, venue: str, rows: list[dict]) -> set[str]:
    transition_dir = Path(root) / venue / "config-transitions"
    markers = sorted(transition_dir.glob("flux-nodes-*-to-*.json"))
    if not markers:
        return set()
    if len(markers) != 1:
        raise CampaignError(
            f"{venue}: expected at most one Flux node transition, found {markers}"
        )
    ...
    required_text = (
        "venue", "cluster",
        "before_controller_env_sha256", "after_controller_env_sha256",
        "before_pipeline_env_sha256", "after_pipeline_env_sha256",
    )
    if marker.get("schema_version") != 1 \
            or marker.get("transition") != "flux_nodes":
        raise CampaignError(f"{marker_path}: unsupported transition schema")
    if marker.get("venue") != venue:
        raise CampaignError(f"{marker_path}: venue does not match its directory")
    for field in required_text:
        if not str(marker.get(field) or "").strip():
            raise CampaignError(f"{marker_path}: {field} is required")
    for field in required_text[2:]:
        if not re.fullmatch(r"[0-9a-f]{64}", str(marker[field])):
            raise CampaignError(f"{marker_path}: {field} is not a SHA-256 digest")

    from_nodes = _positive_int(
        marker.get("from_nodes"), "from_nodes", marker_path
    )
    to_nodes = _positive_int(marker.get("to_nodes"), "to_nodes", marker_path)
    preserved = _positive_int(
        marker.get("completed_runs_preserved"),
        "completed_runs_preserved",
        marker_path,
    )
    if to_nodes >= from_nodes:
        raise CampaignError(f"{marker_path}: Flux transition must reduce nodes")
    if len({row["cluster"] for row in rows}) != 1 \
            or rows[0]["cluster"] != marker["cluster"]:
        raise CampaignError(f"{marker_path}: cluster does not match campaign rows")
    ...
    after_hashes = (
        marker["after_controller_env_sha256"],
        marker["after_pipeline_env_sha256"],
    )
    before_rows = []
    after_rows = []
    for row in rows:
        hashes = (
            row["controller_env_sha256"],
            row["pipeline_env_sha256"],
        )
        if hashes == before_hashes and row["flux_nodes"] == from_nodes:
            before_rows.append(row)
        elif hashes == after_hashes and row["flux_nodes"] == to_nodes:
            after_rows.append(row)
        else:
            raise CampaignError(
                f"{marker_path}: {row['run_dir']} matches neither attested contract"
            )
    if len(before_rows) != preserved:
        raise CampaignError(
            f"{marker_path}: attests {preserved} preserved runs, "
            f"but {len(before_rows)} use the prior contract"
        )
    if not after_rows:
        raise CampaignError(f"{marker_path}: no run uses the corrected contract")
    if any(row["backend"] == "flux" for row in before_rows):
        raise CampaignError(
            f"{marker_path}: a completed Flux run predates its node correction"
        )
    if not any(row["backend"] == "flux" for row in after_rows):
        raise CampaignError(
            f"{marker_path}: the corrected contract has no completed Flux run"
        )
    return {"controller_env_sha256", "pipeline_env_sha256", "flux_nodes"}
...
def validate_campaign(root: Path, through_rep: int, venues=None) -> list[dict]:
    ...
    for venue in venues:
        ...
        transitioned_fields = _flux_node_transition_fields(
            Path(root), venue, venue_rows
        )
        for field in venue_protocol_fields:
            if len({row[field] for row in venue_rows}) != 1:
                if field in transitioned_fields:
                    continue
                non_recovered = [
