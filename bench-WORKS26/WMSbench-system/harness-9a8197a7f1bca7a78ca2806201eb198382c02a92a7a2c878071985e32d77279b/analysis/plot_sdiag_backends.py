#!/usr/bin/env python3
"""Plot Phoenix Slurm context during each backend window (secondary evidence).

The benchmark RPC rate is attributable to the benchmark identity.  Background
...
``--venue`` selects which venue's global context is plotted, and defaults to
Phoenix because that is the shared production venue.
"""
...
from campaign import (BACKEND_DARK, BACKEND_ORDER, CampaignError,
                      add_selection_arguments, load_periodic_sdiag,
                      validate_campaign)
...
PANELS = [
    (("benchmark_rpc_per_min", "Benchmark identity", "o", -0.14, "median"),
...
def collect(root: Path, venue: str, through_rep: int) -> list[dict]:
    ...
    for run in (row for row in campaign if row["venue"] == venue):
        samples = load_periodic_sdiag(Path(run["run_dir"]))
        if samples.empty:
        ...
        rows.append(row)
    expected = len(BACKEND_ORDER) * through_rep
...
def render(out: Path, rows: list[dict], through_rep: int) -> None:
    with plt.rc_context({"font.size": 8}):
        figure, axes_grid = plt.subplots(2, 2, figsize=(7.16, 3.0), sharex=True)
        _render_panels(figure, axes_grid, out, rows, through_rep)


def _render_panels(figure, axes_grid, out: Path, rows: list[dict],
                   through_rep: int) -> None:
    axes = axes_grid.ravel()
    base = np.arange(len(BACKEND_ORDER), dtype=float)
    ...
    for axis, panel in zip(axes, PANELS):
        ...
        axis.set_xticks(
            base, [CONTEXT_BACKEND_LABELS[backend] for backend in BACKEND_ORDER],
        )
...
def main() -> int:
    ...
    parser.add_argument("out", nargs="?", type=Path,
                        default=Path("fig_sdiag_backends_phoenix.png"))
    add_selection_arguments(parser)
    args = parser.parse_args()
    venues = args.venue or ["phoenix"]
    if len(venues) != 1:
        print("select exactly one cluster for the global-context figure",
              file=sys.stderr)
        return 2
    venue = venues[0]
    try:
        rows = collect(args.monitor_root, venue, args.through_rep)
        render(args.out, rows, args.through_rep)
