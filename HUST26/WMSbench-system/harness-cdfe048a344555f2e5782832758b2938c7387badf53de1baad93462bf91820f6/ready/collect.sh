...
#   bash ready/collect.sh -c dev -n 1 --backend=jobarray --allow-sdiag-roll-risk
set -euo pipefail

VENUE=
REP=
BACKENDS=
ALLOW_SDIAG_ROLL_RISK=0
while (( $# )); do
    case "$1" in
        ...
        --backend=*) BACKENDS=${1#*=} ;;
        --allow-sdiag-roll-risk) ALLOW_SDIAG_ROLL_RISK=1 ;;
...
export WMSbench_ALLOW_SDIAG_ROLL_RISK="$ALLOW_SDIAG_ROLL_RISK"
