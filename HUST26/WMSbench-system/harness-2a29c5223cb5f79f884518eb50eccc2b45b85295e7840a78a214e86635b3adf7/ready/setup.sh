...
#   bash ready/setup.sh dev --refresh  # install a new snapshot for later backends
set -Eeuo pipefail
umask 027
...
case ${2:-} in
    "") ;;
    --refresh) REFRESH_SETUP=1 ;;
    *) echo "usage: setup.sh dev|phoenix [--refresh]" >&2; exit 2 ;;
esac
(( $# <= 2 )) || { echo "usage: setup.sh dev|phoenix [--refresh]" >&2; exit 2; }
...
case "$VENUE_INPUT" in
    ...
    phoenix)
        ...
        ALLOCATION_CPUS=28
        LOCAL_CPUS=112
        LOCAL_SBATCH_CPUS=112
        NODE_MEMORY='1500 GB'
        LOCAL_PARTITION=highmem
        LOCAL_QOS=public
        LOCAL_NODELIST=ph001
        HQ_WORKERS=300
        HQ_WORKER_CPUS=26
        FLUX_NODES=32
        ARRAY_SIZE=500
        SLURM_QUEUE_SIZE=10000
        JAVA_MODULE=openjdk-17.0.3_7-4s
        NEXTFLOW_MODULE=nextflow-26.04.0-gcc-14.2.0-qv
        HQ_MODULE=hyperqueue/0.26.2
        MAMBA_MODULE=mamba
        ;;
    *)
        ...
        LOCAL_SBATCH_CPUS=${WMSbench_SITE_LOCAL_SBATCH_CPUS:-$ALLOCATION_CPUS}
        NODE_MEMORY=${WMSbench_SITE_NODE_MEMORY:?set WMSbench_SITE_NODE_MEMORY for $VENUE}
        LOCAL_PARTITION=${WMSbench_SITE_LOCAL_PARTITION:-}
        LOCAL_QOS=${WMSbench_SITE_LOCAL_QOS-}
        LOCAL_NODELIST=${WMSbench_SITE_LOCAL_NODELIST:-}
        HQ_WORKERS=${WMSbench_SITE_HQ_WORKERS:?set WMSbench_SITE_HQ_WORKERS for $VENUE}
        HQ_WORKER_CPUS=${WMSbench_SITE_HQ_WORKER_CPUS:-$LOCAL_CPUS}
        FLUX_NODES=${WMSbench_SITE_FLUX_NODES:?set WMSbench_SITE_FLUX_NODES for $VENUE}
        ARRAY_SIZE=${WMSbench_SITE_ARRAY_SIZE:?set WMSbench_SITE_ARRAY_SIZE for $VENUE}
        SLURM_QUEUE_SIZE=${WMSbench_SITE_SLURM_QUEUE_SIZE:?set WMSbench_SITE_SLURM_QUEUE_SIZE for $VENUE}
        JAVA_MODULE=${WMSbench_SITE_JAVA_MODULE:?set WMSbench_SITE_JAVA_MODULE for $VENUE}
        NEXTFLOW_MODULE=${WMSbench_SITE_NEXTFLOW_MODULE:?set WMSbench_SITE_NEXTFLOW_MODULE for $VENUE}
        HQ_MODULE=${WMSbench_SITE_HQ_MODULE:?set WMSbench_SITE_HQ_MODULE for $VENUE}
        MAMBA_MODULE=${WMSbench_SITE_MAMBA_MODULE:?set WMSbench_SITE_MAMBA_MODULE for $VENUE}
        ;;
esac
...
PARTITION=public
QOS=public
SPECIES_LINE_EXPLICIT=${WMSbench_SITE_SPECIES_LINE+x}
DECLARED_N_EXPLICIT=${WMSbench_SITE_DECLARED_N+x}
SPECIES_LINE=${WMSbench_SITE_SPECIES_LINE:-15}
DECLARED_N=${WMSbench_SITE_DECLARED_N:-1}
[[ $SPECIES_LINE =~ ^[1-9][0-9]*$ ]] || {
    echo "WMSbench_SITE_SPECIES_LINE must be a positive integer" >&2
    exit 2
}
[[ $DECLARED_N =~ ^[1-9][0-9]*$ ]] || {
...
if (( REFRESH_SETUP == 1 )); then
    ACTIVE_LOCK=
    if [[ -d $LOCK_ROOT ]]; then
        ACTIVE_LOCK=$(find "$LOCK_ROOT" -maxdepth 1 -type d \
            -name "$VENUE.*.works26.lock" -print -quit)
    fi
    [[ -z $ACTIVE_LOCK ]] || {
        echo "cannot refresh setup while a collection lock exists: $ACTIVE_LOCK" >&2
        exit 1
    }
    COMPLETED_RUNS=0
    if [[ -d $MONITOR_ROOT/$VENUE ]]; then
        COMPLETED_RUNS=$(find "$MONITOR_ROOT/$VENUE" -path '*/rep*/*/run.json' \
            -type f -print | wc -l)
    fi
    echo "refreshing setup for $VENUE; preserving $COMPLETED_RUNS completed run record(s)"
    rm -f "$CONTROLLER_ENV" "$SETUP_COMPLETE"
fi
...
if [[ -e $CONTROLLER_ENV ]]; then
    ...
    [[ -z $SPECIES_LINE_EXPLICIT \
            || ${WMSbench_SPECIES_LINE:-15} == "$SPECIES_LINE" ]] || {
        echo "installed species-list line is ${WMSbench_SPECIES_LINE:-15}; requested $SPECIES_LINE" >&2
        echo "rerun ready/setup.sh $VENUE --refresh before collecting the next replicate" >&2
        exit 1
    }
    [[ -z $DECLARED_N_EXPLICIT \
            || ${WMSbench_DECLARED_N:-} == "$DECLARED_N" ]] || {
        echo "installed declared N is ${WMSbench_DECLARED_N:-unset}; requested $DECLARED_N" >&2
        echo "rerun ready/setup.sh $VENUE --refresh to change the declared campaign size" >&2
        exit 1
    }
    [[ ${WMSbench_HQ_WORKERS:-} == "$HQ_WORKERS" \
            && ${WMSbench_HQ_WORKER_CPUS:-} == "$HQ_WORKER_CPUS" \
            && ${WMSbench_FLUX_NODES:-} == "$FLUX_NODES" \
            && ${WMSbench_PHYSICAL_NODE_CPUS:-} == "$PHYSICAL_CPUS" \
            && ${WMSbench_ALLOCATION_CPUS:-} == "$ALLOCATION_CPUS" \
            && ${WMSbench_LOCAL_CPUS:-} == "$LOCAL_CPUS" \
            && ${WMSbench_NODE_MEMORY:-} == "$NODE_MEMORY" \
            && ${WMSbench_LOCAL_PARTITION:-} == "$LOCAL_PARTITION" \
            && ${WMSbench_LOCAL_QOS:-} == "$LOCAL_QOS" \
            && ${WMSbench_LOCAL_NODELIST:-} == "$LOCAL_NODELIST" ]] || {
        echo "installed node envelope does not match venue $VENUE; rerun with --refresh" >&2
        exit 1
    }
...
if ! MODULE_PROBE=$(runuser -u "$BENCH_USER" -- env \
    ...
    JAVA_MODULE="$JAVA_MODULE" NEXTFLOW_MODULE="$NEXTFLOW_MODULE" \
    HQ_MODULE="$HQ_MODULE" MAMBA_MODULE="$MAMBA_MODULE" \
...
if ! runuser -u "$BENCH_USER" -- env MODULE_INIT="$MODULE_INIT" \
    MAMBA_MODULE="$MAMBA_MODULE" FLUX_ENV_PATH="$FLUX_ENV_PATH" \
    bash --noprofile --norc -c '
        set -eo pipefail
        set +u
        source "$MODULE_INIT"
...
PARAMS_FILE="$SITE_DIR/params-species-line-$SPECIES_LINE.json"
PARAMS_TMP=$(mktemp "$SITE_DIR/.params-species-line-$SPECIES_LINE.XXXXXX")
cat >"$PARAMS_TMP" <<EOF
{
...
INPUT_MANIFEST="$SITE_DIR/input-manifest-species-line-$SPECIES_LINE.txt"
INPUT_MANIFEST_TMP=$(mktemp "$SITE_DIR/.input-manifest-species-line-$SPECIES_LINE.XXXXXX")
{
...
mv "$INPUT_MANIFEST_TMP" "$INPUT_MANIFEST"
...
PIPELINE_ENV="$SITE_DIR/pipeline-species-line-$SPECIES_LINE.env"
PIPELINE_ENV_TMP=$(mktemp "$SITE_DIR/.pipeline-species-line-$SPECIES_LINE.XXXXXX")
{
    ...
    printf 'export WMSbench_SLURM_QUEUE_SIZE=%q\n' "$SLURM_QUEUE_SIZE"
    printf 'export WMSbench_HQ_WORKERS=%q\n' "$HQ_WORKERS"
    printf 'export WMSbench_HQ_WORKER_CPUS=%q\n' "$HQ_WORKER_CPUS"
    printf 'export WMSbench_FLUX_NODES=%q\n' "$FLUX_NODES"
    printf 'export WMSbench_MODULE_INIT=%q\n' "$MODULE_INIT"
    printf 'WMSbench_MODULES=('
    printf ' %q' "$JAVA_MODULE" "$NEXTFLOW_MODULE" "$HQ_MODULE" "$MAMBA_MODULE"
    printf ' )\n'
...
CONTROLLER_ENV_TMP=$(mktemp "/etc/.WMSbench-controller-$VENUE.env.XXXXXX")
{
    ...
    printf 'export WMSbench_LOCAL_PARTITION=%q\n' "$LOCAL_PARTITION"
    printf 'export WMSbench_LOCAL_QOS=%q\n' "$LOCAL_QOS"
    printf 'export WMSbench_LOCAL_NODE_CONSTRAINT=%q\n' none
    printf 'export WMSbench_LOCAL_NODELIST=%q\n' "$LOCAL_NODELIST"
    printf 'export WMSbench_SPECIES_LINE=%q\n' "$SPECIES_LINE"
    ...
    printf 'WMSbench_SBATCH_COMMON_ARGS=()\n'
...
printf 'HyperQueue worker capacity:  %s CPUs per worker\n' "$HQ_WORKER_CPUS"
printf 'Flux fixed allocation:       %s nodes x %s CPUs\n' \
    "$FLUX_NODES" "$ALLOCATION_CPUS"
printf 'Local placement/envelope:    %s / %s / %s; %s CPUs, %s\n' \
    "${LOCAL_PARTITION:-$PARTITION}" "${LOCAL_QOS:-$QOS}" \
    "${LOCAL_NODELIST:-any node}" "$LOCAL_CPUS" "$NODE_MEMORY"
printf 'frozen species-list line:    %s\n' "$SPECIES_LINE"
