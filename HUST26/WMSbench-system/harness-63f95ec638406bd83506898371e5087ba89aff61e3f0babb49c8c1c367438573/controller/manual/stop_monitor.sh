...
# The safety cap is measured against the operator's cancellation, which is this
# protocol's endpoint.  A pipeline that exited on its own carries no stop
# request, and its own finish marker is the reference instead.
CAP_REFERENCE_EPOCH=${STOP_REQUESTED_EPOCH:-}
[[ -n $CAP_REFERENCE_EPOCH ]] || CAP_REFERENCE_EPOCH=$(env_value "$HANDOFF_DIR/finished.env" finished_epoch)
ENDPOINT_ELAPSED=$(python3 -c 'import sys; print(int(float(sys.argv[1]) - float(sys.argv[2])))' \
    "$CAP_REFERENCE_EPOCH" "$T0_EPOCH")
CAP_EXCEEDED=false
(( ENDPOINT_ELAPSED > WMSbench_CAP_SECONDS )) && CAP_EXCEEDED=true
manual_state_put endpoint_elapsed_s "$ENDPOINT_ELAPSED"
manual_state_put cap_exceeded "$CAP_EXCEEDED"
...
printf '%s\n' \
    ...
    "manual_cap_exceeded=${CAP_EXCEEDED:-false}" \
