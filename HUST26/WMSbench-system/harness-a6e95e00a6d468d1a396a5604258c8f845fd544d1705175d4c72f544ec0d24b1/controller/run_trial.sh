...
required=(
    ...
    WMSbench_ENDPOINT_LOGICAL_KEY_COLUMN
...
"$WMSbench_PYTHON" - "$WMSbench_TRACE_TIMEZONE" <<'PY' || exit 2
import sys
...
if (( PIPELINE_EXIT_CODE != 0 )); then
    echo "pipeline job exited $PIPELINE_EXIT_CODE; see $RUN_DIR/failure-pipeline.stderr and failure-nextflow.log" >&2
    exit 1
fi
