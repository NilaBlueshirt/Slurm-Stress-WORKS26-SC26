...
accepted run contributes one point, its walltime to the LASTZ milestone against
its benchmark-user RPC processing time per 1,000 observed LASTZ tasks, so a
...
def statistics(rows: list[dict], venues: list[str]) -> dict:
    output = {}
    for venue in venues:
...
def nondominated(stats: dict, venue: str) -> list[str]:
    result = []
...
def render(out: Path, rows: list[dict], venues: list[str], expected_n: int) -> None:
    stats = statistics(rows, venues)
    frontiers = {venue: nondominated(stats, venue) for venue in venues}
    ...
    for panel, venue in enumerate(venues):
        ...
        axis.set_xlabel(
            "Hours from clean start $t^{0}$ through the LASTZ milestone",
            fontsize=8,
        )
...
def main() -> int:
    ...
    try:
        ...
        render(args.out, rows, venues, args.through_rep)
