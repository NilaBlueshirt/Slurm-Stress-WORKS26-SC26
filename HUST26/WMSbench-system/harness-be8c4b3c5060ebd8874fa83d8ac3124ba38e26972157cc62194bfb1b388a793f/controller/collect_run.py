...
the completed Nextflow trace into the monitoring tree, records its hash, and
...
# manual runner ends it when the operator, having watched every expected LASTZ
...
def copy_trace(source: Path, destination: Path) -> str:
    if not source.is_file() or source.stat().st_size <= 0:
        raise CollectionError(f"{source}: final Nextflow trace is missing or empty")
    source_hash = sha256(source)
    if destination.exists():
        if not destination.is_file() or sha256(destination) != source_hash:
            raise CollectionError(
                f"{destination}: existing copied trace differs; preserving it"
            )
        return source_hash
    ...
    try:
        ...
        if sha256(temporary) != source_hash:
            raise CollectionError(f"{temporary}: trace copy hash mismatch")
...
def endpoint_task_count(
    trace_path: Path, endpoint_process: str, logical_key: str
) -> int:
    try:
        with trace_path.open(newline="", encoding="utf-8", errors="strict") as handle:
            reader = csv.DictReader(handle, delimiter="\t")
            required = {"process", "status", "exit", logical_key}
            ...
            completed = [
                ...
                and str(row.get("status", "")).strip().upper() == "COMPLETED"
    ...
    if not completed or any(not key for key in completed):
        raise CollectionError(f"{trace_path}: no usable completed endpoint tasks")
    distinct = set(completed)
    if len(distinct) != len(completed):
        raise CollectionError(f"{trace_path}: duplicate successful endpoint tasks")
    return len(distinct)
...
def main() -> int:
    ...
    try:
        ...
        if not re.fullmatch(r"[A-Za-z0-9_.-]+", venue) or backend not in BACKENDS:
            raise CollectionError("unsupported venue/backend")
        ...
        trace_hash = copy_trace(trace_source, run_dir / "trace.txt")
        report_path = run_dir / "report.html"
        if not report_path.is_file() or report_path.stat().st_size <= 0:
            raise CollectionError("centralized Nextflow report is missing or empty")
        ...
        resource_int_fields = (
            ...
            "bulk_nodes", "array_size", "hq_workers", "flux_nodes"
        )
        resource_contract = {
        ...
        endpoint_process = pick(trial, "endpoint_process")
        endpoint_logical_key = pick(
            trial, "endpoint_logical_key_column", "endpoint_logical_key"
        )
        observed_endpoint_tasks = endpoint_task_count(
            run_dir / "trace.txt", endpoint_process, endpoint_logical_key
        )
        record = {
            ...
            "protocol": {
                ...
                "cap_seconds": positive_int(trial, "cap_seconds"),
            },
            ...
            "trace": {
                ...
                "allowed_process_regex": pick(trial, "allowed_process_regex"),
                # A manually stopped pipeline keeps working past LASTZ until
            ...
            "sdiag": {
                ...
                "roll_guard_overridden": pick(
                    trial, "sdiag_roll_guard_overridden",
