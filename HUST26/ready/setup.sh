...
if [[ -e $CONTROLLER_ENV ]]; then
    ...
    [[ -z $SPECIES_LINE_EXPLICIT \
        ...
        echo "rerun ready/setup.sh $VENUE --refresh before collecting the next block" >&2
        exit 1
    }
...
trap - ERR
printf '\nSetup complete. Collect the selected block with:\n'
printf '  bash %q -c %q -n <block-index> --backend=<comma-separated-order>\n' \
