...
def read_trace(run: RunInput) -> tuple[pd.DataFrame, dict]:
    ...
    endpoint_process = str(trace_contract.get("endpoint_process") or "").strip()
    recorded_endpoint_count = _positive_int(
        trace_contract.get(
            "endpoint_task_count",
            trace_contract.get("expected_endpoint_tasks"),
        ),
        "trace.endpoint_task_count", run.run_dir / "run.json",
    )
    if not endpoint_process:
        raise CampaignError(f"{run.run_dir / 'run.json'}: endpoint process is empty")
    allowed_text = str(trace_contract.get("allowed_process_regex") or "").strip()
    if not allowed_text:
        raise CampaignError(f"{run.run_dir / 'run.json'}: allowed process regex is empty")
    try:
        allowed = re.compile(allowed_text)
    except re.error as error:
        raise CampaignError(
            f"{run.run_dir / 'run.json'}: invalid allowed process regex ({error})"
        ) from error

    # A manually stopped run keeps working past LASTZ until the operator
    # cancels it, so its trace carries rows for the following stage.  How far
    # the pipeline got there is operator reaction time, not a property of the
    # deployment strategy, so those rows are separated out and excluded from
    # every measured quantity.  Without a declared post-endpoint set the trace
    # must still contain nothing beyond the allowed processes, which keeps the
    # automated contract exactly as it was.
    post_text = str(trace_contract.get("post_endpoint_process_regex") or "").strip()
    post_endpoint = None
    if post_text:
        try:
            post_endpoint = re.compile(post_text)
        except re.error as error:
            raise CampaignError(
                f"{run.run_dir / 'run.json'}: invalid post-endpoint process regex ({error})"
            ) from error
        if post_endpoint.fullmatch(endpoint_process):
            raise CampaignError(
                f"{run.run_dir / 'run.json'}: the post-endpoint regex matches the "
                f"endpoint process {endpoint_process!r}"
            )
    observed = sorted(set(trace["process"].astype(str)))
    processes = [process for process in observed if allowed.fullmatch(process)]
    post_endpoint_processes = [
        process for process in observed
        if process not in processes
        and post_endpoint is not None and post_endpoint.fullmatch(process)
    ]
    outside = [
        process for process in observed
        if process not in processes and process not in post_endpoint_processes
    ]
    if outside:
        raise CampaignError(f"{source}: processes outside declared set: {outside}")
    post_endpoint_mask = trace["process"].astype(str).isin(post_endpoint_processes)
    post_endpoint_attempts = int(post_endpoint_mask.sum())
    measured = trace.loc[~post_endpoint_mask]
    if measured.empty:
        raise CampaignError(f"{source}: no rows remain after removing post-endpoint work")

    # Retried attempts are normal for this pipeline on a shared cluster and are
    # part of what the deployment strategy costs Slurm.  They are
    # counted and reported, never used to reject a run.  Correctness is decided
    # by the endpoint gate below: every observed logical task must reach
    # COMPLETED exactly once, and campaign validation requires the resulting
    # count to be identical across all backends and replicates.
    statuses = measured["status"].astype(str).str.upper().str.strip()
    exits = measured["exit"].astype(str).str.strip()
    completed_with_bad_exit = statuses.eq("COMPLETED") & ~exits.isin({"0", "0.0"})
    if completed_with_bad_exit.any():
        raise CampaignError(
            f"{source}: {int(completed_with_bad_exit.sum())} COMPLETED rows "
            "have a nonzero or missing exit code"
        )
    failed_mask = statuses.ne("COMPLETED")
    completed = measured.loc[~failed_mask]
    status_counts = {
        str(name): int(count) for name, count in statuses.value_counts().items()
    }
    endpoint_rows = completed.loc[completed["process"].eq(endpoint_process)].copy()
    if endpoint_rows.empty:
        raise CampaignError(f"{source}: no completed rows for {endpoint_process!r}")
    ...
    duplicate_successes = int(len(keys) - distinct_count)
    completion = endpoint_rows["_complete_epoch"]
    missing_completion = int(completion.isna().sum())
    if missing_completion:
        raise CampaignError(
            f"{source}: {missing_completion} successful endpoint rows lack completion time"
        )
    latest_endpoint = float(completion.max())
    if distinct_count != recorded_endpoint_count or duplicate_successes:
        raise CampaignError(
            f"{source}: endpoint distinct/completed rows are "
            f"{distinct_count}/{len(keys)}, recorded {recorded_endpoint_count}"
        )
    ...
    return trace, {
        ...
        "endpoint_task_count": recorded_endpoint_count,
...
def validate_campaign(root: Path, through_rep: int, venues=None) -> list[dict]:
    ...
    processes = {row["endpoint_process"] for row in rows}
    counts = {row["endpoint_task_count"] for row in rows}
    process_sets = {row["trace_processes"] for row in rows}
    logical_keys = {row["endpoint_logical_key_column"] for row in rows}
    allowed_regexes = {row["allowed_process_regex"] for row in rows}
    if (len(processes) != 1 or len(counts) != 1 or len(process_sets) != 1
            or len(logical_keys) != 1 or len(allowed_regexes) != 1):
        raise CampaignError(
            "endpoint identity/count/key/allowed set differs across runs"
        )
    # How far past the endpoint each manually stopped pipeline got before the
    # cancellation landed legitimately varies, so the observed post-endpoint
    # process set is not required to be uniform.  The declared regex that
    # separates it from the measured workload is.  Both runners close the
    # measured window at the post-run boundary, so a block may mix them.
    if len({row["post_endpoint_process_regex"] for row in rows}) != 1:
        raise CampaignError("post-endpoint process regex differs across runs")
    workload_fields = (
