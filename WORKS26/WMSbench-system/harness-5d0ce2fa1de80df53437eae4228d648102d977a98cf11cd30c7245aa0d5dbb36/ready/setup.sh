...
#   bash ready/setup.sh dev --refresh  # only before the first accepted real run
set -Eeuo pipefail
umask 027
...
if (( REFRESH_SETUP == 1 )); then
    ...
    if [[ -d $MONITOR_ROOT/$VENUE ]]; then
        if ! "$CONTROLLER_PYTHON" - "$MONITOR_ROOT/$VENUE" \
                "$BENCH_USER" "$VENUE" <<'PY'
import json
import pathlib
import sys

root, benchmark_user, cluster = pathlib.Path(sys.argv[1]), sys.argv[2], sys.argv[3]
blocked = []
recovered = []
for path in root.glob("rep*/*/run.json"):
    try:
    ...
    if (record.get("benchmark_user") == benchmark_user
        ...
        if reason == "controller_scratch_namespace_manual_import":
...
if recovered:
    print(
        "allowing one-time harness refresh after recorded scratch-namespace "
        "recovery:\n" + "\n".join(map(str, recovered)),
        file=sys.stderr,
    )
PY
        then
            echo "refusing to refresh after a real run was finalized for $VENUE" >&2
            exit 1
        fi
    fi
    echo "refreshing pre-collection setup for $VENUE"
...
if [[ -e $CONTROLLER_ENV ]]; then
    ...
    INSTALLED_FLUX_NODES_OK=0
    if [[ ${WMSbench_FLUX_NODES:-} == "$FLUX_NODES" ]]; then
        INSTALLED_FLUX_NODES_OK=1
    elif [[ $VENUE_INPUT == phoenix && ${WMSbench_FLUX_NODES:-} == 100 \
            && ! -e $WMSbench_MONITOR_ROOT/$VENUE/config-transitions/flux-nodes-100-to-50.json ]]; then
        INSTALLED_FLUX_NODES_OK=1
        echo "reusing the legacy 100-node Phoenix Flux contract; run ready/reconfigure_flux_nodes.sh phoenix before retrying Flux"
    fi
    [[ ${WMSbench_HQ_WORKERS:-} == "$HQ_WORKERS" \
            && ${WMSbench_PHYSICAL_NODE_CPUS:-} == "$PHYSICAL_CPUS" \
            && ${WMSbench_ALLOCATION_CPUS:-} == "$ALLOCATION_CPUS" ]] \
            && (( INSTALLED_FLUX_NODES_OK )) || {
